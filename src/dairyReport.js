...
DairyReport.prototype = {
  ...
  header: function(item) {
    var text = new fabric.Text(item.data, this._styles.header),
        line = new fabric.Rect({left: 0, height: 2, fill: '#00aeb9'});
  ...
  blockList: function(item) {
    this._queue.push(new fabric.Text(item.data.title, this._styles.subHeader));
    var line = new fabric.Rect({left: 0, height: 1, fill: '#00aeb9'}),
        self = this;
    line.isLine = true;
    line.marginBottom = 5;
    ...
    item.data.rows.map(function(row) {
      self._queue.push(new fabric.Text(row, self._styles.text));
      line = new fabric.Rect({left: 0, height: 1, fill: '#E6E6E6'});
      line.isLine = true;
      line.marginBottom = 3;
      self._queue.push(line);
    });
  },
